# test_forest.py
"""
Тест: случайный лес (RandomForestRegressor).
Сильный бейзлайн для табличных данных. Устойчив к неинформативным признакам.
"""
import numpy as np
...
# Настройки леса — оптимизированы для качества
n_estimators = 300
max_depth = None
min_samples_leaf = 1 if rm <= 20 else 2
n_jobs = -1

# Адаптивный выбор max_features
if rm <= 4:
    max_features = 1.0
elif rm <= 20:
    max_features = 0.8
...
test_utils.print_results(
    f"Random Forest (n={n_estimators})", rmse_full, train_time
)
...
var = test_utils.show_comparison(
    ...
    method_name="Random Forest", rm=rm, rmse=rmse_full
)

test_utils.show_surface_3d(im, "Эталон")
test_utils.show_surface_3d(var, "Random Forest")